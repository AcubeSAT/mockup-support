...
void BMP180_Get_Calibration_Params(void); //Read the calibration parameters from the sensor's registers
uint16_t BMP180_Read_Temp_Raw(void); //Get the raw (uncompensated) value of pressure from the sensor
int32_t BMP180_Read_Press_Raw(void); //Get the raw (uncompensated) value of temperature from the sensor
...
//Delay functions declared, for setting delays when required
//Make sure that the variable is not optimized out by setting it to volatile
void _delay_ms(volatile int time)
{
	time *= 8000; //ms according to clock speed
	while(time > 0){time--;}
}
 ...
 */
void BMP180_Send_Command(uint8_t command_register, uint8_t command)
{
	TWIStart(); //Send a start condition to initialize communication
	TWISendAddr(0x77, 1); //Send the write command to the sensor memory
...
void BMP180_Read_Bytes(uint8_t registe, uint8_t *byte_read, uint8_t byte_count)
{
	...
	TWISendAddr(0x77, 0); //Send the read command to the sensor
...
void BMP180_Init(void)
{
		...
		_delay_ms(1); //Delay 10 micro seconds to give some time to the MCU for the I2C interface startup
	#endif
