 ...
 *
 * This module contains drivers for Atmel's AT86RF233 radio device.
 * Made to work with STM32-family microcontrollers
 * (tested on a Nucleo-F446RE dev board, but it should work on other STM32s too
 * due to using HAL functions for communication)
 *
 * **IMPORTANT NOTES BEFORE USE!**
 *
 * Make sure you have initialized the following pins before using the library:
 * 1) PC0: SLEEP (A5)
 * 2) PC1: RESET (A4)
 * 3) PA9: IRQ
 * 4) PA5: SPI_SCK
 ...
 * 7) PA8: SS (D7)
 *
 * @todo        Generalize this for arbitrary pins
 * @todo        Clean up all the dead code in the defines
 *
 * @{
 *
 * @file
 ...
 *
 * @author      Grigoris Pavlakis <grigpavl@ece.auth.gr>  (port to STM32)
 */

#ifndef AT86RF2XX_H_
#define AT86RF2XX_H_

#include "at86rf2xx-defaults.hpp"
#include "at86rf2xx-registers.hpp"
...
class AT86RF2XX {
public:
  volatile static int events; /**< # of pending interrupts from the radio */

  AT86RF2XX(SPI_HandleTypeDef *spiConfig) {
    spi = spiConfig;
  }
   ...
   *
   * If the device does not support the exact dBm value given, it will set a
   * value as close as possible to the given value. If the given value is larger
   * or lower then the maximal or minimal possible value, the min or max value
   * is set, respectively.
   *
   ...
   *
   * This setting specifies the number of attempts to retransmit a frame, when
   * it was not acknowledged by the recipient, before the transaction gets
   * cancelled. The maximum value is 7.
   *
   ...
   *
   * This will force a transition to TRX_OFF regardless of whether the
   * transceiver is currently busy sending or receiving. This function is used
   * to get back to a known state during driver initialization.
   *
   */
   ...
   */
  uint16_t tx_load(uint8_t *data, uint16_t len, uint16_t offset);
   ...
   */
  void rx_read(uint8_t *data, uint16_t len, uint16_t offset);
   ...
   */
  void sram_read(const uint8_t offset, uint8_t *data, const uint16_t len);
   ...
   */
  void sram_write(const uint8_t offset, const uint8_t *data,
                  const uint16_t len);
   ...
   */
  void fb_read(uint8_t *data, const uint16_t len);
