...
int AT86RF2XX::init() {
  uartLog("[at86rf2xx] Booting radio device.");
  ...
  /*  initialize GPIOs */
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_4, GPIO_PIN_SET); // SlaveSelect to high
  // digitalWrite(reset_pin, HIGH);
  // digitalWrite(cs_pin, HIGH);
  // attachInterrupt(digitalPinToInterrupt(int_pin), at86rf2xx_irq_handler,
  // RISING);
  ...
  if (part_num != AT86RF2XX_PARTNUM) {
      uartLog(
        "[at86rf2xx] Error: unable to read correct part number.");
    // Serial.println();
    return -1;
  }

  uint8_t partNoStr[40];
  uartLog("[at86rf2xx] Detected part #");
...
void AT86RF2XX::reset() {
  ...
  // Serial.println();
  uint8_t *resetCompletionStr;
  uartLog("[at86rf2xx] Reset complete.");
}
...
uint16_t AT86RF2XX::send(uint8_t *data, uint16_t len) {
  /* check data length */
  if (len > AT86RF2XX_MAX_PKT_LENGTH) {
    uint8_t *sizeErrorStr;
    uartLog("[at86rf2xx] Error: Data to send exceeds max packet size.");
    // Serial.println();
    return 0;
  }

  uartLog("Transmitting  bytes of data");
...
uint16_t AT86RF2XX::tx_load(uint8_t *data, uint16_t len, uint16_t offset) {
...
uint16_t AT86RF2XX::rx_len() {
  ...
  return (uint16_t)((phr & 0x7f) - 2);
}

void AT86RF2XX::rx_read(uint8_t *data, uint16_t len, uint16_t offset) {
